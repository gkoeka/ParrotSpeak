...
- **Turn-based voice translation** — tap to record, automatic silence-based stop, transcribe → translate → speak back. This is the core (and currently only) live translation flow. Auth, transcription, translation, and the "speak back" (TTS) step are all confirmed working via live device testing as of 2026-07-24; see `CLAUDE.md`'s "Known Issues & Landmines" for the debugging history if this ever regresses. On screen, a turn visibly walks through *uploading → transcribing → translating → preparing audio*, then speaks the translation and logs both the original and translated text. Recordings under 500ms or with no detected speech energy are discarded before upload rather than sent for translation; recordings over 60 seconds trigger a one-time nudge to keep turns shorter, for better accuracy. In manual (non-auto-detect) mode, if the wrong language is detected, the app blocks translation and prompts the user to turn on auto-detect rather than translating gibberish. Languages without on-device speech synthesis fall back to a text-entry box so the conversation can continue by typing.
- **Speech recognition** — runs entirely server-side through OpenAI's `gpt-transcribe`, with the conversation's two configured participant languages passed as a detection hint (not a hard restriction — it still auto-detects between them, which is what powers auto-detect speaker switching). There's no on-device or offline transcription today; see `CLAUDE.md`'s Roadmap for a feasibility spike evaluating on-device Whisper as an offline fallback for no-signal travel conditions.
...
- **Text-to-speech playback** — runs entirely on-device via Expo's speech engine (`expo-speech`), not a cloud voice API — instant and free of an extra network round-trip, at the cost of voice quality being whatever the phone's OS provides. Generic Spanish defaults to Mexican Spanish, generic Portuguese to Brazilian, matching how each language maps to a locale/voice.
...
- **Account & sign-in** — email/password, Google, or Apple Sign-In, all through Clerk. The app's own Postgres database, not Clerk, is the source of truth for subscription status, tier, and any active free-preview window.
