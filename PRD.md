...
*Compiled from a direct code audit of `gkoeka/ParrotSpeak`. Snapshot originally saved 2026-07-22 from the live claude.ai artifact, updated locally 2026-07-24 with that session's fixes (TTS, pricing, Replit decommission) — the claude.ai artifact itself has not been re-synced with these changes yet. Re-sync from the artifact URL above if this file is stale by the time it's read, but check the 2026-07-24 changes in this file's history first since they postdate the artifact.*
