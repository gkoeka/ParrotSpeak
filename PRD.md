...
Model: OpenAI GPT-4o (chat completion, JSON response mode). Runs server-side only — never called from the phone directly. Language display names used in the prompt come from a single canonical lookup (`constants/languageConfiguration.ts`'s `getLanguageByCode`) as of 2026-07-31 — a separately-maintained, drifted code→name map here previously fed raw codes like `"zh"`/`"nl-NL"` into the prompt for several current languages.
Evidence: `server/services/translation.ts:19-32` (prompt), `:102-104` (language-name lookup).

### 3. Speech Recognition — **Live**
Speech-to-text runs entirely through OpenAI's hosted API, on the server. Audio recorded on the phone is base64-encoded and uploaded to the app's own backend, which writes it to a temp file and forwards it to OpenAI's `gpt-transcribe` model (migrated from `whisper-1` 2026-07-31), with the conversation's two configured participant languages passed as a `languages` detection hint — not a hard restriction, still auto-detects between them, which is what powers auto-detect speaker switching (previously no hint was passed at all). There is no on-device or offline transcription of any kind.
Evidence: `server/services/openai.ts:23-77` (`transcribeAudio`), `server/routes.ts` (`/api/transcribe`).
...
76 languages configured (expanded from 65 on 2026-07-30), each with its own record: display name, native name, country/flag, whether it supports speech-to-text, whether it supports text-to-speech, and a coarse "translation quality" tier (high/medium/basic). This drives the text-input fallback — languages flagged as not speech-capable route the user to typing instead of talking. The expansion added real regional variants (Flemish, Belgian/Swiss French, Austrian/Swiss German, European Portuguese — mirroring how Spanish was already split Spain/Latin America) and closed India's Dravidian-language gap (Tamil, Marathi, Gujarati, Kannada, Malayalam — previously zero, despite ~250M+ speakers). "Chinese" was also renamed "Mandarin" to sit clearly alongside the existing separate Cantonese entry.
Evidence: `constants/languageConfiguration.ts` (canonical list, dynamically imported by `server/routes.ts`'s `/api/languages` route).
...
**Today:** OpenAI `gpt-transcribe` (STT) and GPT-4o (translation) called server-side only, through the app's own backend — the OpenAI key never ships to the phone · on-device TTS via `expo-speech`.
...
### 04 — Auth (Clerk) — 🟢 Wired, one real security finding closed 2026-07-30
**Today:** email/password, Google, and Apple sign-in all through Clerk; password reset via Clerk email codes · Clerk identity synced into the app's own DB user row on first request · account deletion (`DELETE /api/user/delete`) now actually implemented — the client had been calling it since before it existed server-side, a silent 404 no-op until fixed.
...
**Gap — resolved 2026-07-30, more serious than the item above:** a full parallel Passport.js session-auth system (separate from the password-reset path already fixed) had coexisted with Clerk this whole time — `/api/auth/login`/`/api/auth/register` were live, unauthenticated, and reachable by anyone on the public internet, completely bypassing Clerk, even though no live client ever used that path. Removed entirely, along with an unwired JWT middleware and a dead WebSocket handler from the same code generation. Fallout found via live device testing (not code review): several client screens had never actually sent a working Clerk token, silently relying on a leftover Passport session cookie now gone — fixed 8 call sites at once via the shared `authenticatedFetch` helper, plus IAP purchase validation specifically (serious — real purchases would have silently failed to validate). The investigation also surfaced pre-existing, unrelated authorization gaps: three GET routes returned every user's data unfiltered when unauthenticated, two routes didn't check that a conversation actually belonged to the requesting user (IDOR), and conversation deletion had no auth check at all. All fixed. Full detail in `CLAUDE.md`'s Known Issues.
...
### 09 — Dependency & Structure Snapshot — 🟢 Native asymmetry resolved 2026-07-30
**Today:** Expo SDK 53 / React Native 0.79.6 / React 19, classic React Navigation (stack + bottom tabs) · full managed/CNG workflow as of 2026-07-30 — both platforms build via `expo prebuild` from `app.config.js` at build time; `android/` was investigated (confirmed pure prebuild output, no real custom native code beyond 3 small config-layer tweaks now made declarative) and removed from git tracking, `/android/` and `/ios/` now gitignored. `expo-doctor` down to 1 failing check from an original 6: fixed 2026-07-29 (`eas-cli`, dead `expo-router`, SDK-53 version bumps, RN Directory false positives) and 2026-07-30 (the Prebuild/CNG conflict, via the managed-workflow decision above).
**Gap:** `expo-av` still flagged unmaintained by React Native Directory (real signal, not fixed) — deferred since it's the exact audio code path fixed and device-tested 2026-07-24, too risky to touch right before Play Store submission. This is now the only failing `expo-doctor` check.
...
~~four overlapping server-side implementations of "call OpenAI to transcribe/translate," only some actually used — not addressed~~ — **fixed 2026-07-31**: collapsed to two live implementations (deleted `server/services/speech.ts` and `server/services/optimizedAudio.ts`, removed a dead `translateText` variant from `server/services/openai.ts`); see item 03 above for the accompanying `whisper-1` → `gpt-transcribe` migration and a language-name lookup bug fixed in the same pass.
...
**Today, also:** ~~the raw-SQL migration runner (`server/db/run-migrations.ts`) was a no-op in production~~ — **fixed 2026-07-30.** It reads `.sql` files from disk at runtime via `fs.readdirSync(path.join(__dirname, 'migrations'))`, but the backend deploys as a single esbuild-bundled `dist/index.js` — the raw `.sql` files never got copied into `dist/`, so that directory didn't exist on Railway and the runner silently found 0 files every boot (why `conversation_metrics` never got created despite its migration files existing before the Railway migration). Build script now copies `server/db/migrations/*.sql` into `dist/migrations/`; verified live in production deploy logs (`[Migr] Found 3 migration file(s)`, was 0 before).
**Gap:**
...
*Compiled from a direct code audit of `gkoeka/ParrotSpeak`. Snapshot originally saved 2026-07-22 from the live claude.ai artifact, updated locally 2026-07-24 (TTS, pricing, Replit decommission), 2026-07-29 (Supabase RLS, `conversation_metrics` table, `expo-doctor` cleanup, dead-file/duplicate-component removal, language-config consolidation), and 2026-07-30 (legacy Passport auth system removed — a real live security hole, not just cleanup; native android/ios asymmetry resolved; migration-runner production no-op fixed; language list expanded 65→76; account deletion implemented) — the claude.ai artifact itself has not been re-synced with any of these changes yet. Re-sync from the artifact URL above if this file is stale by the time it's read, but check this file's history first since it now postdates the artifact by three sessions.*
