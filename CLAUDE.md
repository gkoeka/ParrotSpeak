...
- **NOT YET RESOLVED, actively being debugged as of 2026-07-22: TTS playback throws `Speech synthesis error` and produces no audio whenever the exact requested voice locale isn't available on-device** (e.g. requesting `es-MX`, only an `es-US` voice installed) — despite transcription and translation both succeeding. First hypothesis, already fixed but **confirmed insufficient by a live retest**: `api/speechService.ts`'s `pickPreferredVoice()` returned a fallback voice's `identifier` (e.g. `es-us-x-star05-local`) as `chosen`, while `speakText` still passed the *originally requested* locale (`es-MX`) as `language` — a mismatched pair Android's TTS engine can reject. Fixed by having `pickPreferredVoice` also return `chosenLanguage` (the matched voice's real locale) and having `speakText` use that instead. **Retested live after this fix — still fails with the same error.** Two open questions for next session, in order: (1) confirm the fix actually reached the running bundle (Metro was restarted several times this session; rule out a stale bundle before assuming the code fix itself is wrong — the error log's `for language es-MX` text is *not* proof the fix didn't apply, since that particular log line always prints the originally-requested locale by design, not the actual options passed to `Speech.speak()`); (2) if the bundle is confirmed fresh and it still fails, the real cause may be that the fallback voice identifier itself (`es-us-x-star05-local`) doesn't actually exist on this device — i.e. `Speech.getAvailableVoicesAsync()` may be returning a stale/incorrect list, not just returning a language-mismatched one. Note this may not be Spanish-specific — any language whose exact regional voice isn't installed could hit the same wall.
