...
**Not yet done:**
- **Replit itself has not been decommissioned — hard gate, do not delete the Repl until ALL of the following are true:** (1) an EAS build exists with status `FINISHED` (not `IN_QUEUE`/`IN_PROGRESS`/`ERRORED`) built from a commit that points at `app.parrotspeak.com`; (2) that build has actually been installed on a real device; (3) the user has explicitly confirmed it works correctly on that device (auth, transcription, translation, at minimum). All three are required — a finished build alone is not enough, since an installed build could still fail at runtime. As of 2026-07-22, the dead-secrets cleanup inside Replit (`NEON_DATABASE_URL`, `SUPABASE_DATABASE_URL`, `GITHUB_TOKEN`, `SSH_PRIVATE_KEY_B64`/`SSH_PUBLIC_KEY`, `STRIPE_SECRET_KEY`, `Clerk_OAuth_Application`, `GCloud_Web_App_secret_Clerk_custom_credentials`, `FULLSTORY_ORG_ID`) is done and the Stripe key has been rotated, but the Repl itself is still live and must stay that way until the gate above is satisfied. If asked to delete/decommission the Repl before verifying all three conditions, push back and explain what's still missing rather than proceeding.
