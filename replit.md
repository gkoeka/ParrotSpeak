...
- **Auto-detect Language Sync Fix (Aug 10, 2025)**: Fixed critical bug where participant languages weren't syncing with UI language selections. ConversationScreen now updates ParticipantsContext when source/target languages change, ensuring auto-detect mode correctly routes translations based on UI selections. Previously, participants defaulted to A='en', B='es' regardless of UI showing different languages.
- **Enhanced Recording with 2-Second Rule and Hysteresis**: Advanced silence detection (Jan 10, 2025) - Implements dual-threshold hysteresis system with SPEECH_DB=-50 (speech detection) and SILENCE_DB=-55 (timer clear). Requires 400ms consecutive silence before arming timer. Any sound above -55dB clears armed timer, preventing premature stops during natural speech pauses. Fixed triple timer bug using state machine with inSilence flag and recId guard. Fallback disables auto-stop on devices without metering (Android). Includes sampler logging every 1000ms showing RMS values, consecutive speech/silence durations, and timer state. Logs: "[SilenceTimer] thresholds/armed/reset/elapsed/sample".
- **No-Speech Guard Implementation**: Smart filtering prevents silent recordings from being transcribed. Tracks hadSpeech flag and speechFrames count during recording. If hadSpeech=false, skips transcription API call entirely, saving costs. Service's hadSpeech=true always proceeds regardless of frame count. Trustworthy duration calculation using status.durationMillis or timestamp fallback. Filters recordings <500ms as too short. Logs: "[Filter] speech detected" and "Skipping transcription (no speech)".
...
- **Android Navigation Bar Layout Fix (Jan 10, 2025)**: Fixed safe area handling across the app to prevent UI elements being hidden by Android navigation bars. Removed SafeAreaView wrapper from App.tsx to avoid conflicts. Header component now uses useSafeAreaInsets() instead of hardcoded padding. ConversationScreen uses proper safe area edges ('bottom' only) and simplified bottom padding calculations. Tab bar height respects safe area insets on both iOS and Android platforms.
- **Tab Bar Positioning Fix (Jan 15, 2025)**: Implemented proper footer positioning using React Navigation's BottomTabBar component with custom safe area insets. Tab bar now sits correctly above Android system navigation (3-button, gesture, or hardware buttons) with minimum 24px padding on Android. Added tabBarHideOnKeyboard to hide tab bar when keyboard appears. Solution uses official React Navigation APIs maintaining existing design/colors. Dynamic height calculation: base 60px + bottom safe area.
