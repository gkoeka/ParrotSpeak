...
- **Enhanced Recording with 2-Second Rule**: Continuous silence detection (Jan 10, 2025) - Fixed triple timer bug that caused immediate stops. Now uses state machine with inSilence flag and recId guard. Timer only arms when entering silence (not on speech), clears immediately on speech detection. Works correctly: starts quiet then speaks = no auto-stop, speaking then 2s pause = auto-stop. Fallback disables auto-stop on devices without metering (Android). Logs: "[SilenceTimer] armed/reset/elapsed/cleared".
