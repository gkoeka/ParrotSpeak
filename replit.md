...
- **Tab Navigation Implementation (Feb 5, 2025)**: Implemented bottom tab navigation for mobile-only architecture with four main tabs: Home, Conversations, Pricing, and Settings. Each tab has its own navigation stack with swipe gestures enabled. Tab bar adapts to dark mode with proper styling, platform-specific heights (iOS: 85px, Android: 65px), and shadow effects. Icons use Ionicons with proper active/inactive states. Header removed settings icon since Settings is now accessible via tabs.
- **3-Day Preview Access System (Aug 5, 2025)**: Implemented comprehensive preview access for new users providing 3 days of full app functionality before requiring subscription. Features include: automatic preview setup on registration/OAuth signup, preview status pill in header showing remaining time, 24-hour expiry warning system on HomeScreen, seamless integration with existing subscription middleware, database schema additions (previewExpiresAt, hasUsedPreview, previewStartedAt), updated subscription verification logic to include preview access, and prevention of preview abuse through re-registration tracking. Test accounts configured: koeka@colorado.edu (2 days remaining), greg@gregkoeka.com (12 hours remaining, shows warning), greg.koeka@gmail.com (expired preview), greg@parrotspeak.com (lifetime subscription, unchanged).
- **Always Listening Phase 2 Implementation (Aug 5, 2025)**: Completed Phase 2 of Always Listening feature focusing on speaker switching logic and service integration. Key implementations: AlwaysListeningService now fully integrated with ConversationContext, 7-state conversation state machine (IDLE, LISTENING_SOURCE, PROCESSING_SOURCE, SPEAKING_TRANSLATION, LISTENING_TARGET, PROCESSING_TARGET, SPEAKING_RESPONSE, PAUSED), automatic speaker switching with configurable silence threshold (3 seconds), conversation turn tracking with speaker statistics, mock translation processing with 500ms delay, proper service lifecycle management (initialize/start/pause/stop), comprehensive console logging for debugging state transitions. Integration points: VoiceActivityService callbacks wired to AlwaysListeningService handlers, ConversationContext manages service instance and state updates, AlwaysListeningToggle component properly enables/disables service. Phase 2 test verification shows all components working correctly with proper state transitions and speaker switching.
